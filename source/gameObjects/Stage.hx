	...
	public function new(curStage)
	{
		...
		switch (curStage)
		{
			case 'chase':
				...
				bfrun = new FlxSprite();
				bfrun.frames = Paths.getSparrowAtlas('characters/bf/legs/bfRUN_legs');
				bfrun.antialiasing = true;
				bfrun.animation.addByPrefix('run', 'BFLegs_Run', 24, true);
				bfrun.animation.play('run');
				bfrun.x = 810;
				bfrun.y = 680;
				//bfrun.setGraphicSize(Std.int(not_neded.width * 0.6));
				bfrun.updateHitbox();


			

				//idk why I got this whole jumble of code here but if it aint broke dont fix it
				var goof = new FlxBackdrop(Paths.image('UI/Scrolling_bg/town2'));
				goof.scrollFactor.set();
				goof.velocity.set(1400,0);
				goof.y = 200;
				var motivo = new FlxBackdrop(Paths.image('UI/Scrolling_bg/CHASE wall 2'));
				motivo.scrollFactor.set();
				motivo.velocity.set(2600,0);
				motivo.y = 200;
				var anti = new FlxBackdrop(Paths.image('UI/Scrolling_bg/town1'));
				anti.scrollFactor.set();
				anti.velocity.set(1700,0);
				anti.y = 200;
				var bg = new FlxBackdrop(Paths.image('UI/Scrolling_bg/sky'));
				bg.scrollFactor.set();
				bg.velocity.set(100,0);
				bg.y = 200;
				var floor = new FlxBackdrop(Paths.image('UI/Scrolling_bg/CHASE ground'));
				floor.scrollFactor.set();
				floor.velocity.set(2600,0);
				floor.y = 1000;
